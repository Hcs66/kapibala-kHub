...
### 3.1 IM 框架策略：基于 Matrix/Synapse 构建

**决策：直接使用 Matrix/Synapse 作为 IM 中台底座。**

Matrix 是目前开源生态中唯一具备生产级多平台桥接能力的框架。其 Application Services (AS) 架构、mautrix bridge 家族、事件模型和 Worker 扩展机制，与 kHub 的需求高度吻合。直接使用 Synapse 可以省去自建消息路由、事件持久化、桥接调度等核心基础设施的工作量。

**使用 Matrix/Synapse 获得的能力：**
...
| Transaction 调度器 | 批量事件 + 指数退避重试 + 幂等性保证 | 消息入站的可靠性由 Synapse 保证 |
| 事件模型 | 一切皆事件（消息、成员变更、状态变化），事件持久化到 PostgreSQL | raw_events 天然落库，无需自建事件存储 |
| Worker 架构 | Sync Worker、Event Persister、AS Worker 等可独立扩展 | 按负载拆分 Worker，水平扩展 |
...
| 风险项 | 说明 | 缓解思路 |
|-------|------|---------|
| AGPL-3.0 许可证 | Synapse 采用 AGPL-3.0，要求衍生作品开源。如果 kHub 作为商业中台交付，需要评估许可证合规性 | 方案 A：kHub 业务层与 Synapse 通过 API 交互（非衍生作品）；方案 B：联系 Element 获取商业许可；方案 C：评估 Conduit/Dendrite 等 Apache-2.0 替代实现 |
| 联邦协议过重 | Matrix 设计为去中心化联邦协议，kHub 只需要单向数据收拢，不需要跨服务器联邦能力 | 部署时关闭 Federation（`enable_registration: false` + 不配置 federation listener），仅使用 AS 桥接和本地事件流 |
| Python 运行时性能 | Synapse 主体为 Python，高并发场景下性能不如 Go/Rust 实现 | V1 消息量（~350 msg/min）在 Synapse 舒适区内；如遇瓶颈可迁移到 Dendrite（Go）或 Conduit（Rust） |
...
| 层级 | 技术选型 | 备注 |
|------|---------|------|
| **IM 底座** | Matrix/Synapse | AS 桥接 + 事件模型 + Worker 架构；需在 Checklist 确认许可证 |
...
| 模块 | 交付内容 |
|------|---------|
| **IM 底座** | Synapse 部署与配置（关闭 Federation、AS 桥接、Worker 架构、PostgreSQL 事件存储） |
...
| **Connector** | TG 桥接（基于 mautrix-telegram 或 TS Adapter Shim 封装 TDLib，接入 Synapse AS API）、WA 桥接（基于 mautrix-whatsapp 或 HTTP/WS Adapter 封装 Go 服务 :9800，接入 Synapse AS API） |
...
- Matrix/Synapse 部署验证（关闭 Federation、AS 桥接配置、Worker 架构验证）
- 评估 Matrix 许可证合规方案（AGPL-3.0 影响范围）
- 评估 mautrix-telegram / mautrix-whatsapp 桥接可复用性
- 确认运行时选择（Node.js vs Bun，TDLib binding 验证、Matrix SDK 兼容性验证）
- **产出：技术选型评估报告**

### Phase 2：详细设计（第 2-3 周）

- Synapse 与 kHub 业务层的集成架构设计（AS API 对接、事件订阅、Room/Space 映射）
- Core Layer 数据模型详细设计（Synapse 原生表 + kHub 扩展表：翻译、分析、别名等）
- API 设计（REST API + WebSocket 协议，基于 Matrix Client-Server API 扩展）
- 消息链路详细设计（入站链路 + 出站链路，经 Synapse AS 桥接）
...
- 部署 Synapse 实例（关闭 Federation，配置 AS 桥接，PostgreSQL + Redis）
- 搭建 kHub 业务层开发环境（Node.js + TypeScript + Fastify）
- 部署 MinIO 媒体存储（Docker Compose）
- 编写 kHub 扩展表 migration 脚本（翻译记录、分析结果、别名等）
- 跑通核心链路：TG/WA 消息 → Synapse AS 桥接 → kHub 业务层 → 翻译 → 工作台展示
...
1. **Synapse 集成层**：AS 桥接配置、事件订阅、Room/Space 映射、Worker 部署
2. **Telegram 桥接**：基于 mautrix-telegram 或 TS Adapter Shim 封装 TDLib，接入 Synapse AS API
3. **WhatsApp 桥接**：基于 mautrix-whatsapp 或 HTTP/WS 客户端 Adapter 封装 Go 服务，接入 Synapse AS API
...
8. **WebSocket Gateway**：实时推送（可复用 Synapse /sync 端点或自建 WS 层）、连接管理、心跳、断线重连
...
| 风险项 | 影响 | 概率 | 缓解措施 |
|-------|------|------|---------|
| Synapse AGPL-3.0 许可证合规 | 高 | 高 | 方案 A：API 交互（非衍生作品）；方案 B：Element 商业许可；方案 C：Dendrite/Conduit 替代 |
| Synapse Python 性能瓶颈 | 中 | 低 | V1 消息量在舒适区内；如遇瓶颈可迁移到 Dendrite（Go）或 Conduit（Rust） |
| Matrix 联邦协议引入不必要复杂度 | 中 | 中 | 部署时关闭 Federation，仅使用 AS 桥接和本地事件流 |
...
| 消息量增长超预期 | 高 | 低 | Synapse Worker 架构支持水平扩展；分区策略在架构层面预留 |
...
- [ ] **Matrix/Synapse 作为底座确认**：团队是否认可直接使用 Matrix/Synapse 作为 IM 中台底座？
- [ ] **Matrix 许可证合规性**：Synapse 采用 AGPL-3.0 许可证。如果 kHub 作为商业中台交付，需要评估许可证合规性。是否需要联系 Element 获取商业许可？还是通过 API 交互方式规避衍生作品问题？
- [ ] **Matrix 联邦协议**：确认部署时关闭 Federation 功能，仅使用 AS 桥接和本地事件流。是否接受这种配置？
- [ ] **Matrix 性能评估**：V1 消息量（~350 msg/min）在 Synapse 舒适区内。如果未来消息量增长，是否接受迁移到 Dendrite（Go）或 Conduit（Rust）的可能性？
...
- [ ] **Matrix SDK 选择**：使用 matrix-js-sdk（官方）还是 matrix-bot-sdk（社区）？是否需要评估两者的功能差异和维护状态？
...
- [ ] **PostgreSQL 版本**：生产环境 PostgreSQL 版本是否 ≥ 14？（Synapse 推荐 14+，声明式分区 + pg_trgm 需要）
- [ ] **Redis 版本**：生产环境 Redis 版本是否 ≥ 6.2？（Synapse Worker 通信 + Pub/Sub 特性需要）
