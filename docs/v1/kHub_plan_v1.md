...
#### Tuwunel（RocksDB）与 kHub 业务层（PostgreSQL）的数据同步策略

**核心原则：kHub 业务层永远不直接访问 Tuwunel 的 RocksDB，所有数据交互通过 Matrix 标准 API 完成。**

Tuwunel 的 RocksDB 是内嵌存储引擎，不暴露 SQL 接口，也没有 CDC（Change Data Capture）机制。这与 Synapse 使用 PostgreSQL 的情况有本质区别 — Synapse 方案下理论上可以直接查询 homeserver 的 PostgreSQL 表（虽然不推荐），而 Tuwunel 方案下这条路完全不通。
...
外部 IM 平台（TG/WA）
        │
        ▼
  mautrix bridge（AS 进程）
        │ AS Transaction API（HTTP PUT）
        ▼
  Tuwunel Homeserver ──── RocksDB（Matrix 协议数据，kHub 不直接访问）
        │
        │ 两条数据出口 ↓
        │
   ┌────┴─────────────────────┐
   │                          │
   ▼                          ▼
AS Transaction Push      Client-Server /sync
（推模式，推荐）          （拉模式，备选）
   │                          │
   ▼                          ▼
kHub Event Consumer（Node.js）
   │
   ▼
PostgreSQL（kHub 扩展表：翻译、分析、别名、画像等）
```

**两条数据出口对比：**

| 维度 | AS Transaction Push（推荐） | Client-Server /sync |
|------|---------------------------|---------------------|
| **机制** | Homeserver 主动将匹配 AS 命名空间的事件批量推送到 AS 注册的 HTTP 端点 | kHub 作为 Matrix 客户端，通过长轮询 `/sync` 端点拉取增量事件 |
| **实时性** | 准实时（事件产生后毫秒级推送） | 近实时（取决于 /sync 轮询间隔，通常 30s 超时长轮询） |
| **可靠性** | Homeserver 保证 at-least-once 投递，失败时指数退避重试，支持幂等性（transaction ID） | 客户端需自行维护 `since` token，断线后从上次 token 恢复 |
| **过滤能力** | 通过 AS 注册文件的 `namespaces` 声明过滤（用户、房间、别名） | 通过 Filter API 精细过滤事件类型、房间、发送者 |
| **适用场景** | 服务端后台消费，高吞吐，kHub 主链路 | 客户端场景，或作为 AS Push 的补充/降级方案 |
| **开发量** | kHub 注册为 AS，实现 HTTP 端点接收 transaction | 使用 matrix-js-sdk 或 matrix-bot-sdk 的 /sync 循环 |

**推荐方案：kHub 注册为 Application Service（AS）**

kHub 业务层自身注册为一个 Matrix Application Service，通过 AS Transaction API 接收所有桥接事件。这是 mautrix bridge 家族使用的同一套机制，成熟可靠。

工作流程：
1. kHub 在 homeserver 注册 AS 配置文件（`khub-as.yaml`），声明关注的命名空间（所有桥接用户和房间）
2. Tuwunel 收到 mautrix bridge 推送的事件后，持久化到 RocksDB，同时将事件通过 HTTP PUT 推送到 kHub 的 AS 端点
3. kHub Event Consumer 接收事件，提取消息内容、发送者、房间等信息，写入 PostgreSQL 扩展表
4. 后续的翻译、分析、画像等业务流程基于 PostgreSQL 中的数据触发

**Synapse vs Tuwunel 方案下的数据访问差异：**

| 维度 | Synapse（PostgreSQL） | Tuwunel（RocksDB） |
|------|----------------------|---------------------|
| **直接查询 homeserver 数据** | 理论上可行（共享 PostgreSQL），但不推荐（schema 不稳定，升级可能破坏） | 不可行（RocksDB 无 SQL 接口，内嵌在 Tuwunel 进程内） |
| **AS Transaction Push** | ✅ 支持 | ✅ 支持（行为一致） |
| **Client-Server /sync** | ✅ 支持 | ✅ 支持（行为一致） |
| **kHub 扩展表存储** | 可与 homeserver 共用同一 PostgreSQL 实例（不同 schema） | 必须独立 PostgreSQL 实例 |
| **历史消息回填** | 可通过 `/messages` API 分页拉取，或直接查 PostgreSQL | 只能通过 `/messages` API 分页拉取 |
| **运维复杂度** | 一个 PostgreSQL 实例（共享），但需注意 homeserver 升级对 schema 的影响 | 两套存储（RocksDB + PostgreSQL），但职责清晰，互不干扰 |

**关键设计决策：无论选择 Synapse 还是 Tuwunel，kHub 业务层都应通过 Matrix 标准 API（AS Push + Client-Server API）获取数据，不直接访问 homeserver 内部存储。** 这样做的好处是：
1. homeserver 实现可替换（Synapse ↔ Tuwunel），业务层零改动
2. 不依赖 homeserver 内部 schema，升级无风险
3. 架构边界清晰：homeserver 负责协议层，PostgreSQL 负责业务层

**需在 Checklist 中确认：**

| 确认项 | 说明 |
|-------|------|
| AS 注册配置 | kHub 作为 AS 注册到 Tuwunel 的配置方式和命名空间声明，需在 Phase 1 验证 |
| 事件投递性能 | Tuwunel AS Transaction Push 在 V1 消息量（~350 msg/min）下的投递延迟和吞吐量 |
| 历史消息回填 | kHub 首次部署时，如何通过 `/messages` API 回填已有会话的历史消息到 PostgreSQL |
| PostgreSQL 独立实例规划 | Tuwunel 方案下 kHub 扩展表的 PostgreSQL 实例规格、备份策略、与 Tuwunel RocksDB 的部署拓扑 |
...
| **Connector** | 外部 IM 平台与 Matrix homeserver 之间的桥接层，负责协议转换、身份映射和媒体转存。设计说明见下方。 |
...
Connector 层是外部 IM 平台（Telegram、WhatsApp）与 Matrix homeserver 之间的桥接层。kHub 选择基于 Matrix 的 Application Services（AS）桥接架构，而非自建消息路由，核心原因是 mautrix bridge 家族已提供生产级的协议转换实现，可大幅降低开发量。

V1 采用 Puppeting 模式：每个受控账号以真实身份登录 bridge，所有会话自动桥接为 Matrix Room 事件，kHub 业务层通过订阅 homeserver 事件流获取消息。

每个平台的 Connector 有两条可选路径，需在 Phase 1 验证后确定：

**Telegram Connector：**

| 路径 | 方案 | 说明 |
|------|------|------|
| Plan A | mautrix-telegram 直接部署 | 成熟的 Python 桥接实现，注册为 Matrix AS，自动完成 TG 协议转换、用户映射、媒体转存。kHub 零开发量，仅需配置。 |
| Plan B | TS Adapter Shim + TDLib | 复用现有 TDLib bindings 代码，编写 Adapter Shim（约 300-500 行 TS）将 TDLib 事件映射为 Matrix AS 入站事件格式。适用于 mautrix-telegram 不满足定制需求的场景。 |

**WhatsApp Connector：**

| 路径 | 方案 | 说明 |
|------|------|------|
| Plan A | mautrix-whatsapp 直接部署 | Go 实现的桥接，通过 WhatsApp Multi-Device API 接入，注册为 Matrix AS。同样零开发量，仅需配置。 |
| Plan B | HTTP/WS Adapter Shim + Go 服务 | 复用现有 Go 服务（:9800），编写 Adapter Shim（约 300-500 行 TS）通过 HTTP/WebSocket 调用 Go 服务，将响应映射为 Matrix AS 入站事件格式。 |

**Connector 层核心职责：**

- **协议转换**：将平台特定的消息格式（TG 的 `Update` 对象、WA 的 protobuf 消息）转换为 Matrix 事件模型
- **身份映射**：平台用户 ID 映射为 Matrix 虚拟用户（ghost user），保持发送者身份在 Matrix 侧的一致性
- **媒体转存**：收到媒体消息后立即从平台下载并转存至 MinIO 对象存储，不依赖平台原始 URL 做持久化访问
- **会话映射**：平台聊天/群组自动映射为 Matrix Room，群组层级映射为 Matrix Space
- **连接管理**：维护受控账号的登录 session，健康检查（每 30 秒轮询连接状态），断连自动重连（指数退避）
- **能力声明**：通过 PlatformCapabilityManifest 声明平台差异（如 TG 支持消息编辑而 WA 不支持），上层业务按能力适配

**与 TDR 中 ConnectorAdapter 接口的关系：**

TDR（kHub_tdr_v1.md §10）设计了 `ConnectorAdapter` 接口和 `ConnectorRegistry` 注册机制。在引入 Matrix homeserver 后，如果选择 Plan A（mautrix 直接部署），Connector 层的协议转换、身份映射、事件格式化等工作由 mautrix bridge 完成，kHub 业务层直接消费 Matrix 事件，`ConnectorAdapter` 接口退化为可选的薄封装层。如果选择 Plan B（Adapter Shim），则 `ConnectorAdapter` 接口仍然适用，Shim 实现该接口并将事件注入 Matrix AS API。

**扩展性设计：**

- 新平台接入（如 LINE）只需部署对应的 mautrix bridge 或编写新的 Adapter Shim，Core 层无需修改
- `ConnectorRegistry` 模式保留，支持配置驱动的 Connector 实例化和生命周期管理
- V2 预留热加载能力（无需全量重启即可接入新 Connector）
