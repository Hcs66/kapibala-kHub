# 统一 IM 会话中台 (kHub) — 产品规划与技术选型报告 - V1
...
| 竞品类别 | 代表产品 | 与我们的核心差异 |
|---------|---------|----------------|
| 企业级多渠道客服 | Intercom、Zendesk、Chatwoot | 以工单/agent 工作流为核心，IM 只是渠道之一；我们是数据中台，暴露消息管道而非锁定在客服产品内 |
| 跨境电商营销 IM | Respond.io、SleekFlow、WATI | 封闭式 SaaS，数据不可导出；我们提供底层基础设施，这类产品可以建在我们上面 |
| IM 聚合平台 | Matrix/Beeper、Matterbridge | Matrix 是通用联邦协议，过重；Matterbridge 极简但无持久化/翻译；我们只做单向数据收拢，不需要联邦能力 |

**定位结论**：kHub 不是客服产品，不是营销工具，不是聊天软件。它是面向业务系统(和开发者)的 IM 数据中台 — 解决"消息从哪来、怎么存、如何用"。

**关键借鉴**：unified inbox 交互模式（← 客服平台）、联系人数据标准化驱动上层业务（← 营销工具）、AS 桥接架构 + thin connector 设计（← Matrix）。
