...
- [15. Matrix 客户端形态预研](#15-matrix-客户端形态预研v123-新增)
...
> **kHub = IM 中间平台 + 销售工作台**
>
> kHub 的职责是：把多平台 IM Connector 接入进来，经过 Tracy 已有的翻译/分析管道处理，最终通过实时工作台呈现给销售员。数据中台能力（存储、翻译、分析、客户管理）由 Tracy Core API 提供。
>
> **⚠️ 客户端形态待定：** 销售工作台的客户端形态（独立 Web SPA / Electron / 自研客户端 / Matrix 体系）尚未确定，需碰面后决策。V1 后端先行，前端/客户端实施冻结。
...
| 销售工作台 Web UI | React SPA、会话列表、消息视图、翻译预览、分析侧栏 | ~8,000 行 TSX | **🧊 冻结：客户端形态未定，碰面后启动** |

**C 档总计：~20,500 行 TS/TSX**（其中 ~8,000 行 TSX 冻结，可立即推进的后端部分 ~12,500 行 TS）
...
| 10 | ~~Tauri 桌面壳~~ 客户端形态（Web SPA / Electron / 自研 / Matrix） | 待碰面决策 | 客户端形态未定，需先确认再启动前端实施 |
| 11 | 移动端 / PWA | V2+ | 待业务验证 |
| 12 | 客户画像 LLM | V2+ | 非核心 |
| 13 | 离线模式 | V2+ | 非核心 |
| 14 | 全局快捷键 | V2 | 非核心 |
...
| 前端框架 | React 19 + TypeScript | **⚠️ 待定：客户端形态未确认** |
| 状态管理 | Zustand | **⚠️ 待定** |
| UI 组件库 | shadcn/ui + Tailwind CSS | **⚠️ 待定** |
| 构建工具 | Vite | **⚠️ 待定** |
| 虚拟滚动 | @tanstack/virtual | **⚠️ 待定** |
| 部署架构 | 模块化单体 | — |
...
┌─────────────────────────────────────────────────────────────┐
│                    Client Layer（⚠️ 形态待定）                 │
│    碰面后确认：Web SPA / Electron / 自研客户端 / Matrix 体系    │
│   ┌──────────────────┐                                       │
│   │  销售工作台        │  域名/入口待定                        │
│   └────────┬─────────┘                                       │
└────────────┼────────────────────────────────────────────────┘
...
> **🧊 本节冻结（v1.2.3）**
>
> **冻结理由：** 客户端形态未定。CTO 反馈销售工作台可能不走 Web SPA 方向（Electron 也存疑），需碰面后确认客户端形态（独立 Web SPA / Electron / 自研客户端 / Matrix 体系）再启动前端实施。以下内容保留作为参考，但不作为实施依据。
>
> **冻结范围：** 布局、虚拟滚动消息列表、翻译预览、AI 分析侧栏、会话切换不重连 WS、入站默认未读、Connector 断连提示、数据脱敏显示、会话内搜索、原文/译文切换、输入框/发送/预览交互（~8,000 行 TSX）。
>
> **解冻条件：** 碰面确认客户端形态后，根据决策结果重写或调整本节。

**独立部署：**
- 域名：`workbench.huidu.ai`（⚠️ 待定，取决于客户端形态）
...
|------|---------|------------|---------|-------------|
| D-01 | Postgres 共享 + message_raw 增强方案 C | 2-3 人天 | 阻塞 Phase 2 数据层最后一步（2.1.5） | kHub 能连接共享 PG，message_raw 新字段可写 |
| D-02 | Ingest 路由增强（conversation_seq + platform_msg_id 强去重） | 1-2 人天 | 阻塞 Phase 2 入站联调（2.4.2） | POST /ingest 返回 conversation_seq，重复 platform_msg_id 返回 409 |
| D-03 | RBAC 加 grant_visibility_policy capability | 1 人天 | 阻塞 Phase 4 验证（4.3.1） | supervisor/boss 能授予/撤销 visibility policy |
...
| D-06 | Plugin Config 加 khub.* 命名空间 | 0.5 人天 | 阻塞 Phase 3.5 | GET /plugin/config/resolved 返回 khub.* 配置项 |
| D-07 | trace_id 透传 X-Trace-Id header | 0.5 人天 | 阻塞 Phase 1 trace_id 端到端验证 | Tracy response header 包含 X-Trace-Id |
| D-08 | Admin SPA 加 2 Page（Visibility 管理 + Account Health） | 3-4 人天 | 阻塞 Phase 4.3 端到端验证（等 D-03 后可做） | Visibility 管理 Page + Account Health Page |
...
> **优先级说明（v1.2.3）：** 1.3 TDLib 兼容性验证为 Phase 1 最高优先级，必须最先打掉技术风险，独立于一切 UI 决策。
...
| 3.1.2 | 实现鉴权：建连时 JWT 验签 + role=sales 校验 | 鉴权可用 | **⚠️ 后端逻辑可做；鉴权流细节（客户端预持 JWT 直连 vs HTTP 登录拿 token）待碰面对齐** |
...
| 3.4.4 | CORS 配置（允许 workbench.huidu.ai + admin.huidu.ai） | 跨域可用 | **⚠️ 域名待定：如果走客户端形态可能不需要 CORS，碰面后确认** |
...
> **⚠️ 拆分说明（v1.2.3）：** 后端 API（4.2.1-4.2.7）可独立推进，与客户端形态无关。但多账号管理的 UI 交互流（添加账号、扫码登录、断连恢复提示、切换账号）在客户端 vs Web 里完全两套，**UI 部分冻结，待碰面后确认客户端形态再设计**。
...
| `workbench.huidu.ai` | kHub 销售工作台 | **⚠️ 待定：取决于客户端形态决策，如果走客户端可能不需要独立域名** |
| `api.huidu.ai` | Tracy Core API | 不动，kHub 后端也调这个 |
| `wss://ws.huidu.ai` | kHub WebSocket Gateway | **⚠️ 待定：同上，取决于客户端形态** |
...
### 13.2 待碰面决议事项（v1.2.3 新增，按风险从高到低排序）

| # | 待决议事项 | 影响范围 | 状态 |
|---|-----------|---------|------|
| 7 | **销售工作台客户端形态**：独立 Web SPA / Electron 壳 / 自研客户端 / Matrix 体系 / 多形态都做？ | §6.7 整节、§3.4 CORS、§10.6 域名规划 | ⚠️ 待碰面 |
| 8 | **登录架构**：客户端/壳负责登录后注入 JWT，还是 SPA 自己跳 Tracy /auth/login？ | §3.1.2 WS 鉴权流、Store 设计 | ⚠️ 待碰面 |
| 9 | **多账号管理 UI 归属**：账号增删/切换 UI 放在客户端还是 SPA？ | §4.2 Account Core 的 UI 流形态 | ⚠️ 待碰面 |
| 10 | **桌面级能力 V1 范围**：托盘、原生通知、全局快捷键、自动更新——哪些 V1 必须？哪些 V2？ | 直接决定客户端选型边界 | ⚠️ 待碰面 |
| 11 | **多开/多窗口需求**：是否 V1 必须？多开是同一进程多窗口、还是多账号多实例？ | UI 框架选型、状态管理拆分 | ⚠️ 待碰面 |
| 12 | **BullMQ 是否独立**：kHub 和 Tracy 各自独立 BullMQ，需确认无冲突 | 部署架构 | ⚠️ 待确认 |
| 13 | **域名规划**：`workbench.huidu.ai` 是否仍保留？如果走客户端则可能不需要 | 部署、CORS | ⚠️ 待碰面 |

### 13.3 跨方依赖（等 CTO 推代码）

| D 档编号 | 改造内容 | 精确阻塞点 | 状态 |
|---------|---------|-----------|------|
| D-01 | message_raw 增强方案 C | 阻塞 Phase 2 数据层最后一步（2.1.5） | 等 CTO |
| D-02 | Ingest 路由增强（conversation_seq + 强去重） | 阻塞 Phase 2 入站联调（2.4.2） | 等 CTO |
| D-03 | RBAC grant_visibility_policy capability | 阻塞 Phase 4 验证（4.3.1） | 等 CTO |
| D-06 | Plugin Config 加 khub.* 命名空间 | 阻塞 Phase 3.5 | 等 CTO |
| D-07 | trace_id 透传 X-Trace-Id header | 阻塞 Phase 1 trace_id 端到端验证 | 等 CTO |
| D-08 | Admin SPA 加 2 Page（Visibility + Account Health） | 阻塞 Phase 4.3 端到端验证 | 等 CTO 推 D-03 后可做 |
...
| Tauri 桌面客户端 | V2（待重估） | 客户端形态待碰面决策，Tauri 仅为候选之一 |
...
## 15. Matrix 客户端形态预研（v1.2.3 新增）

> **性质：** 非阻塞预研任务，与后端 Phase 1-2 并行推进，碰面时带方案讨论。

**背景：** CTO 反馈 Electron 后续可能不再用（局限和 bug 太多），原本提 Electron 只是想借鉴它的界面思路（如对话多开等交互形态），不是绑定其技术架构。建议调研 Matrix 生态作为替代方向。

**预研目标：**

| # | 调研项 | 产出 |
|---|--------|------|
| 1 | Matrix 是否有现成的、完善的前端框架或客户端可以借鉴？ | 候选方案列表 + 优劣对比 |
| 2 | Matrix 客户端形态是怎样的（Web / Desktop / Mobile / 多端同步）？ | 形态分析报告 |
| 3 | 如果借鉴 Matrix 的协议或客户端实现，与 kHub 当前的 WS + REST 架构如何融合？需要哪些改造？ | 融合可行性评估 + 改造清单 |

**决策原则：** 从用户场景反推（销售员的实际工作流：多账号、多对话、长时间挂机、桌面通知、跨设备），选择最稳的客户端形态，再决定 §6.7 的实施方式。
